...
function isPowerOfTwo(num) {
  return 2 ** Number.parseInt(Math.log2(num), 10) === num;
}
...
function toFixed(number, fractionDigits) {
  if (Number.isInteger(number)) {
    return Number.parseInt(number, 10).toFixed(fractionDigits);
  }
  return Number.parseFloat(number, 10).toFixed(fractionDigits);
}
